...
Helpers used by the GLASS pipeline (Snakemake rules and `start.sh`).

## Snakemake rule scripts (called by the Snakefile)
...
- **position_utils.sh** – Position/PDB helpers: `parse_positions`, `get_cysteine_positions`, `check_chain_in_pdb`, `get_sequence_bounds`, `is_terminal_position`, `get_residue_name_at_position`, `is_cysteine`, `find_nglyc_sequon_starts`. Requires `GLASS_ROOT` set to the `local_run` directory.
